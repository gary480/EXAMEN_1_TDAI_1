...
rutas.put('/actualizar/:id', async(req, res) => {
    try {
        ...
        if (!clienteactualizado) {
            ...
            return res.status(201).json(Datos_Actualizados);
        }
...
//cantidad declientes agregados
rutas.get('/cantidad', async(req, res) => {
    try {
        const cant = await clientemodel.countDocuments();
        return res.json({ Los_clientes_Registrados_son: cant });
    } catch (error) {
        res.status(500).json({ mensaje: error.message })
    }
});
